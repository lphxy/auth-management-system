...
public class SSOFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        String clientSessionId = CookieUtil.getCookie(request, WAN_UPMS_CLIENT_SESSION_ID);
        if (StringUtils.isEmpty(clientSessionId)) {
            clientSessionId = request.getSession().getId();
            CookieUtil.setCookie(response, WAN_UPMS_CLIENT_SESSION_ID, clientSessionId);
        }

        // 判断局部会话是否登录
        if (null != clientSessionId && !StringUtils.isEmpty(RedisUtil.get(WAN_UPMS_CLIENT_SESSION_ID + "_" + clientSessionId))) {
            // 移除url中的token参数
        ...
        // 未登录
                ...
                try {
                    ...
                    if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                        ...
                        if (result.equals("success")) {
                            // token校验正确，创建局部会话
                            RedisUtil.set(WAN_UPMS_CLIENT_SESSION_ID + "_" + clientSessionId, token);
                            // 保存token对应的局部会话sessionId，方便退出登录操作
                            RedisUtil.getJedis().sadd(WAN_UPMS_CLIENT_SESSION_IDS + "_" + token, clientSessionId);
                            logger.info("当前token={}，对应的注册系统个数：{}个", token, RedisUtil.getJedis().scard(WAN_UPMS_CLIENT_SESSION_IDS + "_" + token));
            ...
            logger.info("未登录，跳转认证中心:{}", sso_server_url);
